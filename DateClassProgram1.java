...
public class DateClassProgram1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(d1.getTime()); // system understandale time in long format ( for human understanding use
											// epochconverter.com )

		Date d2 = new Date(d1.getTime()+(1000*60*60*24*1)); // human understandale time  (+ for tommorows time and - for yest's time)
		...
		String format3 = date.concat("-").concat(month).concat("-").concat(year); // concatinating everything to print
																					// 28-May-2025

		System.out.println(format3);

		String format4 = date.concat("/").concat(month).concat("/").concat(year); // concatinating everything to print
																					// 28/May/2025
